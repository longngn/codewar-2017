...
class Tile(object):
    ...
    def isNearBase(self, player):
        ...
        return False
    def distToBaseCorner(self, player): return self.distToSomeTile(player, lambda u: u.isBaseCorner(player))
...
class Player(object):
    ...
    def getNumberOfUnstableTiles(self):
        ...
        return res
    def viableTiles(self): return [t for t in self.pos.adjacentTiles() if table.get(t) != self.unstableId]
class MyPlayer(Player):
    def moveToAdjTile(self, tile):
        move = Tile.getDirection(self.pos, tile)
        Game.writeMove(move)
    def moveToBase(self):
        nextTile = self.distToBase()[1]
...
while True:
    ...
    if me.inBaseCorner():
        ...
        for t in me.viableTiles():
            ...
            elif t != me.lastPos and t.isBase(me): back = t
